   ...
   @Subscribe
    public void onSync(EventSync e) {
        ...
        switch (targetLogic.getValue()) {
            case HP -> target = CrackedClient.combatManager.getTargetByHealth(targetRange.getValue());
            case Distance -> target = CrackedClient.combatManager.getNearestTarget(targetRange.getValue());
            case FOV -> target = CrackedClient.combatManager.getTargetByFOV(targetRange.getValue());
        }
        ...
        if (e.getEntity() instanceof EndCrystalEntity crystal && !placedCrystals.isEmpty()) {
            ...
            for (BlockPos bp : cache.keySet())
                if (crystal.squaredDistanceTo(bp.toCenterPos()) < 0.3 && timing.getValue() == Timing.NORMAL && breakTimer.passedMs(breakDelay.getValue())) {
                    ...
                    CrackedClient.asyncManager.run(() -> handleSpawn(crystal));
                }
        }
    }
    ...
    public void onBlockDestruct(EventSetBlockState e) {
        ...
        if (target != null && target.squaredDistanceTo(e.getPos().toCenterPos()) <= 4 && e.getState().isAir() && !e.getPrevState().isAir() && blockRecalcTimer.every(200)) {
            ...
            CrackedClient.asyncManager.run(() -> calcPosition(recalculate.getValue() == Recalc.FAST ? 2f : placeRange.getValue(), recalculate.getValue() == Recalc.FAST ? e.getPos().toCenterPos() : mc.player.getPos()));
        }
    }
    ...
    public boolean shouldPause() {
        ...
        return isEnabled() && switchPause.getValue() && !CrackedClient.playerManager.switchTimer.passedMs(switchDelay.getValue()) && !silent && !silentWeakness;
    }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        for (Entity ent : entities) {
            ...
            if (protectFriends.getValue()) {
                ...
                for (PlayerEntity pl : players) {
                    if (!CrackedClient.friendManager.isFriend(pl)) continue;
    ...
    public void onTick(EventTick e) {
        ...
        CrackedClient.asyncManager.run(() -> {
