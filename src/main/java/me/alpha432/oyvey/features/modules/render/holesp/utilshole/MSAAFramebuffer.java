...
public class MSAAFramebuffer extends Framebuffer {
    ...
    public static void use(boolean fancy, Runnable drawAction) {
        ...
        //use(Math.min(fancy ? 16 : 4, MAX_SAMPLES), MinecraftClient.getInstance().getFramebuffer(), drawAction);//disable
        use(Math.min(fancy ? 16 : 2, MAX_SAMPLES), MinecraftClient.getInstance().getFramebuffer(), drawAction);//Enable
        //use(Math.min(MAX_SAMPLES, MAX_SAMPLES), MinecraftClient.getInstance().getFramebuffer(), drawAction);
    }
