...
public class CrackedPop extends Module {
    ...
    public Setting<Integer> messageLength = this.register(new Setting<Integer>("MessageLag", 200, 1, 1000));
    ...
    public void onReceivePacket(PacketEvent.Receive event) {
        ...
        if ((entity.equals(mc.player) && totemsIgnoreOwn.getValue())
            || (CrackedClient.friendManager.isFriend(((PlayerEntity) entity)) && totemsIgnoreOthers.getValue())
            || (!CrackedClient.friendManager.isFriend(((PlayerEntity) entity)) && totemsIgnoreFriends.getValue())
        ) return;
        ...
        synchronized (totemPopMap) {
            ...
            if (!ignoreFriends.getValue() || (ignoreFriends.getValue() && !CrackedClient.friendManager.isFriend((PlayerEntity)entity))) {
                if (poplag.getValue()){
...
private String generateLagMessage() {//Generate message lagg
    String message = null;
