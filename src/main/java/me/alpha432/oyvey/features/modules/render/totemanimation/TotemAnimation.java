package me.alpha432.oyvey.features.modules.render.totemanimation;
...
public class TotemAnimation extends Module {
    private static TotemAnimation INSTANCE = new TotemAnimation();
    public TotemAnimation() {
        super("TotemAnimation", "TotemAnimation", Module.Category.RENDER, true, false, false);
        this.setInstance();
    }

    public static TotemAnimation getInstance() {
        if (INSTANCE == null) {
            INSTANCE = new TotemAnimation();
        }
        return INSTANCE;
    }
    ...
    public Setting<Mode> mode = this.register(new Setting<Mode>("Mode",Mode.FadeOut));
