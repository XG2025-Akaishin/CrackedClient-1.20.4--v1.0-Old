...
public class Notifications
    ...
    public String getNotifierOn(Module module) {
        if (ModuleTools.getInstance().isEnabled()) {
            switch (ModuleTools.getInstance().notifier.getValue()) {
				case CRACKED: {
                    String text = (CrackedClient.commandManager.getClientMessage()) + " " + Formatting.BOLD + module.getDisplayName() + Formatting.RESET + Formatting.GREEN + " enabled.";
                    return text;
                }
                case FUTURE: {
        ...
        String text = CrackedClient.commandManager.getClientMessage() + " " + Formatting.GREEN + module.getDisplayName() + " toggled on.";
        return text;
    }
    ...
    public String getNotifierOff(Module module) {
        if (ModuleTools.getInstance().isEnabled()) {
            switch (ModuleTools.getInstance().notifier.getValue()) {
				case CRACKED: {
                    String text = (CrackedClient.commandManager.getClientMessage()) + " " + Formatting.BOLD + module.getDisplayName() + Formatting.RESET + Formatting.RED + " disabled.";
                    return text;
                }
                case FUTURE: {
        ...
        String text = CrackedClient.commandManager.getClientMessage() + " " + Formatting.RED + module.getDisplayName() + " toggled off.";
        return text;
    }
    ...
    public void onToggleModule(ClientEvent event) {
        ...
        if (event.getStage() == 0 && !(module = (Module)event.getFeature()).equals(this) && (modules.contains(module.getDisplayName()) || !((Boolean)this.list.getValue()).booleanValue())) {
                Command.sendMessageId(getNotifierOff(module), -1);
            //chatHud.addMessage(Text.literal(String.format("%s",getNotifierOff(module))));
        }
        ...
        if (event.getStage() == 1 && (modules.contains((module = (Module)event.getFeature()).getDisplayName()) || !((Boolean)this.list.getValue()).booleanValue())) {
                Command.sendMessageId(getNotifierOn(module), -1);
            //chatHud.addMessage(Text.literal(String.format("%s",getNotifierOn(module))));
        }
    }
