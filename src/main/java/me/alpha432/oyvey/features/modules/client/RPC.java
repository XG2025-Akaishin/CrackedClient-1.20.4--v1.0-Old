...
public class RPC extends Module {
    ...
    public void startRpc() {
        ...
        if (!started) {
            ...
            presence.largeImageText = CrackedClient.NAME + " v" + CrackedClient.VERSION + " by " + "AnarchyGooD ";
            ...
            thread = new Thread(() -> {
                while (!Thread.currentThread().isInterrupted()) {
                    ...
                    switch (smode.getValue()) {
                        case Stats ->
                                presence.state = "Hacks: " + CrackedClient.moduleManager.getEnabledModules().size() + " / " + CrackedClient.moduleManager.modules.size();
                        case Custom -> presence.state = state.getValue();
                        case Version -> presence.state = CrackedClient.NAME + " v2.0 - 1.20.4";
                    }
