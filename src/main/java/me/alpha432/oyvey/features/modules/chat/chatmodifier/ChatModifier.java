...
public class ChatModifier extends Module {
    ...
    private Setting<String> spamerstring = this.register(new Setting("CustomSpammer", "CustomSpam"));
    ...
    public void onPacketSend(PacketEvent.Send event) {
        if (event.getPacket() instanceof ChatMessageC2SPacket packet) {
            ...
            string = COLORTEXT + COLOR + packet.chatMessage() + SUFFIX; // posible compatibilidad de encrypt base64 aes 
            mc.player.networkHandler.sendChatMessage(COLORTEXT + COLOR + packet.chatMessage() + SUFFIX); // posible adaptacion a la string por probar
            event.setCancelled(true);//Disable /() Cancel == evitar errores
        }
    }
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        if (event.getPacket() instanceof GameMessageS2CPacket pac && globalchat.getValue()) {
            ...
            switch (this.globalmode.getValue()) {
				case CRACKED: {
                    GLOBAL = (CrackedClient.commandManager.getClientMessage()) + " " + Formatting.RESET;
                    break;
                }
            }
            switch (this.globalmode.getValue()) {
   ...
   public void onTick() {
    ...
    public String getDisplayInfo() {
        return globalmode.getValue().name();
    }
}
