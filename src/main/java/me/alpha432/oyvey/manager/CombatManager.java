...
import me.alpha432.oyvey.event.impl.autototem.AutoTotemPopEvent;
...
public class CombatManager implements IManager {
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        ...
        if (event.getPacket() instanceof EntityStatusS2CPacket pac) {
            if (pac.getStatus() == EntityStatuses.USE_TOTEM_OF_UNDYING) {
                ...
                EVENT_BUS.post(new AutoTotemPopEvent((PlayerEntity) ent, popList.get(ent.getName().getString())));
            }
        }
    }
    ...
    public List<PlayerEntity> getTargets(float range) {
        return mc.world.getPlayers().stream()
                .filter(e -> !e.isDead())
                .filter(entityPlayer -> !CrackedClient.friendManager.isFriend(entityPlayer.getName().getString()))
